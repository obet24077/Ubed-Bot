...
let handler = async (m, { conn, args, __dirname, usedPrefix, command }) => {
    try {
        ...
        if (/audio/.test(mime)) {
            ...
            exec(`ffmpeg -i "${media}" ${set} -acodec libmp3lame "${filename}"`, async (err, stderr, stdout) => {
                unlinkSync(media);
                if (err) throw '*Gagal mengubah audio!*';

                if (!existsSync(filename) || statSync(filename).size === 0) {
                    throw '*Gagal mengubah audio, file kosong atau rusak*';
                }
        ...
        } else throw `*[❗NOTE]* Balas audio/vn yang ingin dimodifikasi dengan perintah ${usedPrefix + command}`;
    } catch (e) {
        throw e;
    }
};

handler.help = ['bass', 'blown', 'deep', 'earrape', 'fast', 'fat', 'nightcore', 'reverse', 'robot', 'slow', 'tupai', 'reverb', 'chorus', 'flanger', 'distortion', 'pitch', 'highpass', 'lowpass', 'underwater'].map(v => v + ' [vn]');
handler.tags = ['audio'];
handler.command = /^(bass|blown|deep|earrape|fas?t|fat|nightcore|reverse|robot|slow|tupai|squirrel|chipmunk|reverb|chorus|flanger|distortion|pitch|highpass|lowpass|underwater)$/i;
