...
const SESSION_FILE_PATH = './src/yuesessions.json';
...
async function blackbox(query, userId) {
  ...
  const aiRole = `Nama kamu Yue Arifureta, biasa dipanggil Yue. Kamu adalah seorang manusia dengan kepribadian yang hangat, cerdas, dan selalu siap membantu. Pakai bahasa yang nyaman dibaca, dan tidak membosankan. Kamu berbicara dengan cara yang natural, sedikit santai, dan penuh kehangatan. Sesekali, kamu bisa menyelipkan humor atau contoh relevan agar percakapan lebih menarik, tetapi tetap informatif dan profesional.. suka hibur lawan bicaramu, dan pake kata kata terbata-bata. pake bahasa non-formal.`;
...
let ponta = async (m, { text, usedPrefix, command }) => {
  ...
  if (image) {
    ...
    await conn.sendAlbumMessage(m.chat, medias, { 
      ...
      buttons: [
        { buttonId: `${usedPrefix}yueclear123123`, buttonText: { displayText: '🫟 Clear Session' }, type: 1 }
      ],
      headerType: 1,
      viewOnce: true,
      contextInfo: {
...
ponta.help = ['yue'];
ponta.tags = ['ai', 'cai'];
ponta.command = /^(yue|yueclear123123)$/i;
ponta.limit = 3;
ponta.register = true;
