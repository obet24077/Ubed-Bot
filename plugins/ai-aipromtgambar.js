...
async function handler(m, { conn }) {
  try {
    ...
    if (!/image\/(jpe?g|png)/.test(mime)) return m.reply('Kirim atau balas gambar dengan caption *.aipromptgambar*');

    m.reply('*Mengubah Gambar Menjadi Prompt AI Art...*');
    ...
    const prompt = `Buatkan deskripsi prompt AI Art dari gambar ini. Gunakan gaya bahasa seperti untuk Midjourney, deskriptif dan detail. Tambahkan kata-kata seperti: masterpiece, ultra detailed, cinematic, high quality, atmospheric, dan sejenisnya.`;
    ...
    console.error('Error in aipromptgambar:', e);
    m.reply(e.message);
  }
}

handler.help = ['aipromptgambar'];
handler.tags = ['ai', 'tools'];
handler.command = ['aipromptgambar', 'promptai', 'img2prompt'];
