...
class SnakeAndLadderGame {
    ...
    startGame = async (m, player1Name, player2Name) => {
        ...
        const { key } = await conn.sendMessage(m.chat, { image: await this.getBoardBuffer() });
        this.keyId = key;
    };
...
const handler = async (m, { args, usedPrefix, command }) => {
    ...
    switch (args[0]) {
        ...
        case 'start':
            ...
            conn.ulartangga[m.chat] = { ...conn.ulartangga[m.chat], state: true };
        ...
        case 'reset':
            conn.ulartangga[m.chat] = { ...conn.ulartangga[m.chat], state: false };
