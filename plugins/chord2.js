...
let handler = async (m, { text }) => {
	if (!text) throw 'Masukkan query pencarian liriknya!'
	let a = await lirikSaja(text)
	m.reply(`*Lagu :* ${a.judul}\n*Lirik :*\n\n${a.lirik}`)
}

handler.help = ['lirik']
handler.tags = ['internet']
handler.command = /^(chord2)$/i

handler.limit = true

export default handler

export async function lirikSaja(query) {
  return new Promise(async(resolve, reject) => {
    ...
    axios.get("http://app.chordindonesia.com/?json=get_post&id="+data.posts[0].id, {
      ...
      resolve({
        judul: $("img").attr("alt"),
        lirik: $("pre").text().trim().replace(/.*?/g, '')  // Menghapus notasi chord seperti [C], [Am]
      });
    }).catch(reject);
  });
}