...
let handler = async (m, {
    ...
    if (!input) {
        ...
        const transformedDataResult = await Promise.all(transformedData.map(async (item, index) => ({
            ...item,
            avatar: await conn.profilePictureUrl(dataArray[index], "image").catch(_ => null)
        })));
...
`.trim()
    ...
    await conn.sendMessage(m.chat, { text: caption, mentions: conn.parseMention(caption) }, { quoted: m })
} else if (args[0] == '3') {
            // Inventory 3
