...
let handler = async (m, { conn, command, usedPrefix, text }) => {
  ...
  if (inputcmd === "start") {
    ...
    const eris = global.db.data.users[m.sender].eris || 0;
    players.push({ name: playerName, eris, sender: m.sender });
...
function shuffleRoles(players) {
  const shuffledPlayers = players.map((player, index) => ({ name: player, role: roles[1], eris: 0, sender: '' }));
