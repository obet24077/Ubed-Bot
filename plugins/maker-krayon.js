...
let handler = async (m, { conn }) => {
  ...
  if (!mime || !/image\/(jpe?g|png)/.test(mime)) {
    return m.reply('❌ Kirim atau balas gambar dengan caption *.crayon*')
  }
  ...
  try {
    ...
    const apiUrl = `https://api.ubed.my.id/imgedit/crayon?apikey=ubed2407&imageUrl=${encodeURIComponent(imageUrl)}`
    ...
    await conn.sendMessage(m.chat, {
      image: buffer,
      caption: '✅ *Gambar berhasil diedit dengan efek crayon!*',
    }, { quoted: m })

    await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })

  } catch (e) {
    console.error('CRAYON API Error:', e)
    await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
